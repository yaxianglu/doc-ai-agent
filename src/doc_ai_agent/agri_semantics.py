...
def extract_crop_hint(text: str, context_crop: str = "") -> str:
    """从问题或上下文里提取作物提示。"""
    normalized = str(text or "")
    for token in CROP_HINTS:
        if token in normalized:
            return token
    return str(context_crop or "").strip()


def extract_scene_hint(text: str, context_scene: str = "") -> str:
    """从问题或上下文里提取生产场景提示。"""
    normalized = str(text or "")
    for token, label in SCENE_HINT_ALIASES.items():
        if token in normalized:
            return label
    return str(context_scene or "").strip()


def needs_crop_scene_clarification(text: str, context: dict | None = None) -> bool:
    """判断泛化农业建议/解释是否需要先补充作物或场景。"""
    normalized = str(text or "")
    context = dict(context or {})
    has_generic_guidance = any(token in normalized for token in ["注意什么", "注意哪些", "要注意什么", "该注意什么"])
    if not (needs_advice(normalized) or needs_explanation(normalized) or has_generic_guidance):
        return False
    if any(token in normalized for token in ["虫情", "虫害", "墒情", "低墒", "高墒", "预警", "报警", "告警"]):
        return False
    if extract_crop_hint(normalized, str(context.get("crop") or "")):
        return False
    if extract_scene_hint(normalized, str(context.get("scene") or "")):
        return False
    return any(token in normalized for token in ["台风", "暴雨", "灾害", "受灾", "天气"])
...
def has_detail_intent(text: str) -> bool:
    ...
    if any(token in normalized for token in DETAIL_HINTS):
        return True
    if any(token in normalized for token in ["明细", "按天", "逐天", "列出来"]):
        return True
    if "数据" in normalized and not has_overview_intent(normalized):
        if any(token in normalized for token in ["从数据看", "基于数据看", "数据上看"]):
            return False
        if any(token in normalized for token in ["虫情", "虫害", "墒情"]):
            return True
        return any(token in normalized for token in ["给我", "看", "查", "查询", "列出", "展示", "导出", "具体", "详细", "原始"])
    return False
