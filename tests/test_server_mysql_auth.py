...
class ServerMySQLAuthTests(unittest.TestCase):
    ...
    def test_mysql_runtime_uses_mysql_repo_without_mutating_auth_users(
        self,
        mysql_repo_cls,
        auth_service_cls,
        _source_provider,
        ...
        auth_service_cls.return_value.ensure_users.assert_not_called()
    ...
    @patch("doc_ai_agent.server.load_source_provider", return_value=None)
    @patch("doc_ai_agent.server.AuthService")
    def test_non_mysql_runtime_still_seeds_memory_auth(
        self,
        auth_service_cls,
        _source_provider,
        _playbook_router,
